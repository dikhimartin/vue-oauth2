...
<div v-if="user.role === 'admin'">
  ...
  >
    <div class="row">
      <div class="col-md-4">
        <form @submit.prevent="updateUserSchedule" enctype="multipart/form-data">
          <div
          ...
          >
            ...
            <select class="form-control" v-model="user_schedules.user_id" disabled="disabled">
          ...
          <button type="submit" class="btn btn-primary">Add Jadwal Peserta</button>
        </form>
      </div>
    </div>
  </box>
...
export default {
  ...
  mounted() {
    ...
    this.Api.get("/user_schedules_details/" + this.$route.params.id).then((res) => {
      // window.console.log(res.data.schedules)
      this.user_schedules = res.data.user_schedules[0];
    });
  ...
  methods: {
    ...
    updateUserSchedule(e) {
      ...
      this.Api.post("/user_schedules/" + this.$route.params.id, {
        user_id: this.user_schedules.user_id,
        schedule_id: this.user_schedules.schedule_id,
        _method: 'PUT',        
        }).then((response) => {
